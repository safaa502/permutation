...
const LoginScreen = ({ navigation, setUserLoggedIn }) => {
  ...
  const handleLogin = async () => {
    try {
      ...
      if (isEmailPresent) {
        Alert.alert("Adresse e-mail valide", "Bienvenue sur votre compte professeur.");
        setUserLoggedIn(true); // Mettre à jour l'état de connexion de l'utilisateur
        // Naviguer vers la page de l'utilisateur connecté
        navigation.navigate('UserProfile', { email });
      } else {
        Alert.alert("Adresse e-mail non valide");
      }
